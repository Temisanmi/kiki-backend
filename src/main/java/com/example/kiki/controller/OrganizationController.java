...
public class OrganizationController {
    ...
    @PreAuthorize("hasRole('ORGANIZATION')")
    public ResponseEntity<Page<OrgOrderItemDto>> getMyOrders(
            @PageableDefault(sort = "order.createdAt", direction = Sort.Direction.DESC) Pageable pageable
    ){
        return ResponseEntity.ok(organizationService.getMyOrderItems(pageable));
    }
}